...
class XSeq(pp.Sequence):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        lims = pp.Opts(
            ...
            max_slew=max_slew,
            slew_unit="T/m/s",
            rf_ringdown_time=rf_ringdown_time * 1e-6,
    ...
    def add_spec_events(self):
        ...
        # Make rf event
        self.spec.rf, self.spec.gz, self.spec.gz_r = pp.make_sinc_pulse(
            flip_angle=self.spec.flip,
            return_gz=True,
            system=self.system,
            use="excitation",
        ...
        # Make adc event
        self.spec.adc = pp.make_adc(
            num_samples=self.spec.size,
            system=self.system,
    ...
    def add_met(
        ...
        **kwargs,
        ...
        met_obj.size_acq = np.int32(
            np.ceil(
                np.array(met_obj.size) * np.array([1, met_obj.pf_pe, met_obj.pf_pe2])
            )
        )
        ...
        flat_time = (
            np.ceil(t_acq / self.system.grad_raster_time) * self.system.grad_raster_time
        )
        if self.ramp_samp is False:
            met_obj.gx = pp.make_trapezoid(
                channel="x",
                system=self.system,
                flat_time=flat_time,
            ...
            if (
                gy_dur > (met_obj.gx.rise_time + met_obj.gx.fall_time)
                and self.symm_ro is True
            ):
                raise RuntimeError(
                    "Phase encoding blip cannot fit in between readout plateaus"
                )
            ...
            try:
                ...
                met_obj.gx = pp.make_trapezoid(
                    "x", area=gx_area, duration=min_dur, system=self.system
                )
            ...
            met_obj.gx.amplitude = (
                met_obj.gx.amplitude / gx_samp_area_slow * gx_samp_area
            )
            ramp_time = met_obj.gx.rise_time + met_obj.gx.fall_time
            met_obj.gx.area = met_obj.gx.amplitude * (
                met_obj.gx.flat_time + 0.5 * ramp_time
            )
            ...
            met_obj.dwell = (
                np.floor(rs_dwell / self.system.adc_raster_time)
        ...
        if self.symm_ro is False:
            met_obj.fly = pp.make_trapezoid(
                "x", system=self.system, area=-met_obj.gx.area
            )
        ...
        if self.ramp_samp is False:
            ...
            adc_delay = (
                met_obj.gx.rise_time
                + met_obj.gx.flat_time / 2
        ...
        met_obj.gx_pre = pp.make_trapezoid(
            "x", system=self.system, area=-met_obj.gx.area / 2
        )
        ...
        if np.abs(gy_pre_area) > 0:
            met_obj.gy_pre = pp.make_trapezoid(
                "y", system=self.system, area=gy_pre_area
            )
        else:
            met_obj.gy_pre = pp.make_trapezoid(
                "y", system=self.system, area=-met_obj.gx.area / 2
            )
        ...
        if met_obj.size[2] > 1 and self.acq_3d is True:
            ...
            met_obj.gz_pre = pp.make_trapezoid(
                self.slice_axis, system=self.system, area=gz_pre_area
            )
        ...
        if self.symm_ro is True:
            ...
            gy_up, gy_dn, _ = pp.align(
                right=gy_split[0], left=[gy_split[1], met_obj.gx]
            )
        ...
        if self.grad_spoil is True:
            ...
            met_obj.x_spoil = pp.make_trapezoid(
                "x", system=self.system, area=spoil_area
            )
            met_obj.y_spoil = pp.make_trapezoid(
                "y", system=self.system, area=spoil_area
            )
            if self.acq_3d is True and met_obj.size[2] > 1:
                ...
                met_obj.z_spoil = pp.make_trapezoid(
                    self.slice_axis, system=self.system, area=spoil_area
                )
        ...
        if self.acq_3d is True:
            met_obj.z_range = np.linspace(1, -1, met_obj.size[2])[
                0 : int(met_obj.size_acq[2])
            ][::-1]
            if met_obj.z_centric is True:
                z_sort = np.argsort(np.abs(met_obj.z_range))
                met_obj.z_range = met_obj.z_range[z_sort]
        else:
            met_obj.z_range = np.arange(
                -np.floor(met_obj.size[2] / 2), np.ceil(met_obj.size[2] / 2)
            )
        ...
        # Create sinc events
            ...
            met_obj.rf, met_obj.rf_gz, met_obj.rf_gz_r = pp.make_sinc_pulse(
                flip_angle=met_obj.flip,
                return_gz=True,
                system=self.system,
    ...
    def create_ssrf_rf_ev(self, met_idx):
        ...
        rf_end = (
            np.ceil(rf_end / self.system.grad_raster_time)
            * self.system.grad_raster_time
        )
        met_obj.mag = interp_waveform(
            mag, met_obj.rf_delta, self.system.rf_raster_time, ti_end=rf_end
        )
        met_obj.pha = interp_waveform(
            pha, met_obj.rf_delta, self.system.rf_raster_time, ti_end=rf_end
        )

        # Construct rf event
        pha_off = 2 * np.pi * met_obj.kz * self.slc_off
        ssrf = met_obj.mag * np.exp(1j * (met_obj.pha + pha_off))
        met_obj.rf = pp.make_arbitrary_rf(
            ssrf,
            2 * np.pi,
            system=self.system,
            norm=False,
        ...
        delay_aug = (
            np.ceil(rf_dur / self.system.block_duration_raster)
            * self.system.block_duration_raster
        )
        if delay_aug > 0:
    ...
    def create_ssrf_grad_ev(self, met_idx):
        ...
        grd_data *= (
            met_obj.grd_max / np.max(np.abs(grd_data)) * 1e-2 * self.system.gamma
        )
        ...
        grd_data_i = interp_waveform(
            grd_data * (not self.no_slc),
            met_obj.grd_delta,
            self.system.grad_raster_time,
        )
        ...
        met_obj.kz = interp_waveform(
            kz, met_obj.grd_delta, self.system.rf_raster_time, ti_end=kz_end
        )
                ...
                for m in range(self.n_met):
                    ...
                    if r > 0:
                        ...
                        if self.alt_pha is True:
                            ...
                            met.gy_blip_amp *= -1
                        if (
                            self.alt_slc is True
                            and self.acq_3d is True
                            and met.size[2] > 1
                        ):
                            met.gz_pre_amp *= -1
                    ...
                    for z_idx, z in enumerate(met.z_range):
                        ...
                        if met.use_sinc is False:
                            ...
                            try:
                                self.add_blck_lbl(
                                    met.rf_gz, met.rf, met.rf_delay, lbl=m
                                )
                            except AttributeError:
                            ...
                            if echo == 0:
                                if self.acq_3d is False or met.size[2] == 1:
                                    ...
                                    if met.use_sinc is False:
                                        ...
                                        self.add_blck_lbl(
                                            met.gx_pre, met.gy_pre, met.gz_pre, lbl=m
                                        )
                                    else:
                                        self.add_blck_lbl(
                                            met.gx_pre, met.gy_pre, met.gz_pre, lbl=m
                            ...
                            for i in range(met.size_acq[1]):
                                ...
                                if self.symm_ro is True and met.size_acq[1] > 1:
                                    if i == met.size_acq[1] - 1:
                                        self.add_blck_lbl(
                                            met.adc, met.gx, met.gy_dn, lbl=m
                                        )
                                    elif i == 0:
                                        self.add_blck_lbl(
                                            met.adc, met.gx, met.gy_up, lbl=m
                                        )
                                    else:
                                        self.add_blck_lbl(
                                            met.adc, met.gx, met.gy, lbl=m
                                        )
                                else:
                                ...
                                cond_2 = (
                                    i == met.size_acq[1] - 1 and echo != self.n_echo - 1
                                )
                        ...
                        if self.grad_spoil is True:
                            met.x_spoil.amplitude = met.spoil_amp * np.sign(
                                met.gx.amplitude
                            )
                            if self.symm_ro is True:
                                met.y_spoil.amplitude = met.spoil_amp
                            else:
                                met.y_spoil.amplitude = met.spoil_amp * np.sign(
                                    met.gy.amplitude
                                )
                            if self.acq_3d is True and met.size[2] > 1:
                                met.z_spoil.amplitude = -met.gz_pre.amplitude
                            self.add_blck_lbl(
                                met.x_spoil, met.y_spoil, met.z_spoil, lbl=m
                            )
                        ...
                        if self.tr > 0:
                            ...
                            tr_delay = (
                                np.ceil(tr_delay / self.system.block_duration_raster)
                                * self.system.block_duration_raster
                            )
                            if tr_delay > 0:
    ...
    def create_param_dic(self):
        ...
        # Generic parameters
        out_dic = {
            "fov": [int(dim * 1e3) for dim in self.fov],
            "rbw": self.rbw,
            "n_avg": self.n_avg,
            "n_rep": self.n_rep,
            "tr": self.tr * 1e3,
